...
fn sparkleHash(v: vec3<f32>) -> f32 {
    let d = dot(v, vec3<f32>(12.9898, 78.233, 37.719));
...
fn vs_main(input: VSIn) -> VSOut {
    var out: VSOut;
    out.position = applyProjectionWarp(u.projection * u.view * vec4<f32>(input.position, 1.0));
    out.sparkle = sparkleHash(input.position);
    return out;
}
